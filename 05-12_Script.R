...
############################################################################
# Note: For the results of this study, only line 1-343 are relevant.
...
library(circlize)
hcd<-color_labels(hcd,4, col = c(pal1))
hcd<-color_branches(hcd,4, col = c(pal1))
...
'# extract statements based on cluster 1-15: phrase matching (keyword-in-context) --> 536 matches with sparse = 0.45

cluster1_kw <- kwic(toks_all_tagged_corp, pattern = phrase("electron* servic*"), valuetype = "regex", window = 3) # 12 matches
cluster2_kw <- kwic(toks_all_tagged_corp, pattern = phrase("provid* trust*"), valuetype = "regex", window = 3) # 33 matches
cluster3_kw <- kwic(toks_all_tagged_corp, pattern = phrase("member* state* identif*"), valuetype = "regex", window = 3) # 1 match
cluster4_kw <- kwic(toks_all_tagged_corp, pattern = phrase("data* requir*"), valuetype = "regex", window = 3) # 8 matches
cluster5_kw <- kwic(toks_all_tagged_corp, pattern = phrase("european* regul*"), valuetype = "regex", window = 2) # 7 matches
cluster6_kw <- kwic(toks_all_tagged_corp, pattern = phrase("digit* ident*"), valuetype = "regex", window = 3) # 408 matches
cluster7_kw <- kwic(toks_all_tagged_corp, pattern = phrase("public* secur*"), valuetype = "regex", window = 3) # 2 matches
cluster8_kw <- kwic(toks_all_tagged_corp, pattern = phrase("commiss* implement*"), valuetype = "regex", window = 3) # 17 matches
cluster9_kw <- kwic(toks_all_tagged_corp, pattern = phrase("nation* standard*"), valuetype = "regex", window = 3) # 2 matches
cluster10_kw <- kwic(toks_all_tagged_corp, pattern = phrase("sector* option*"), valuetype = "regex", window = 3) # 1 match
cluster11_kw <- kwic(toks_all_tagged_corp, pattern = phrase("develop* system*"), valuetype = "regex", window = 3) # 2 matches
cluster12_kw <- kwic(toks_all_tagged_corp, pattern = phrase("framework* privat*"), valuetype = "regex", window = 3) # 7 matches
cluster13_kw <- kwic(toks_all_tagged_corp, pattern = phrase("eida* eid*"), valuetype = "regex", window = 3) # 36 matches'
